...
with st.sidebar:
    ...
    if (option == "Post Generation"):
        optionpg1 = st.selectbox(
            'Choose Social Media', ('Linkedin', 'Twitter/X'))

        # In Post Generation , we select Linkedin
        if (optionpg1 == "Linkedin"):
            style = st.selectbox('Choose Post Style', ('Professional',
                                 'Friendly', 'Creative', 'Inspirational', 'Storytelling'))
            domain = st.text_input('Your Working Domain/Field')
            optionlp = st.selectbox('Select length of post you want?', (
                'small -  approx 150 words', 'medium -  approx 350 words', 'long -  approx 500 words'))

         # In Post Generation , we select twitter
        if (optionpg1 == "Twitter/X"):
            style1 = st.selectbox('Choose Post Style', ('Professional',
                                  'Friendly', 'Creative', 'Inspirational', 'Storytelling'))
            optiontp = st.selectbox('Select length of post you want?', (
                'small -  approx 50 words', 'medium -  approx 150 words', 'long -  approx 250 words'))
    ...
    st.markdown("[Github](https://github.com/shweta0408)")
